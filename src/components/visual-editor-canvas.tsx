...
const RenderedCanvasElement: React.FC<VisualEditorCanvasProps & { element: CanvasElement; parentId?: string | null }> = ({
  ...
  const renderInnerContent = (el: CanvasElement) => {
    ...
    switch (el.type) {
      ...
      case "Image":
        const isLocalUpload = el.src?.startsWith('/secret1/');
        let imageEffectiveSrc = el.src || "https://placehold.co/300x200/eee/ccc?text=Нет+URL";

        if (isLocalUpload && imageEffectiveSrc) {
          // Add cache-busting query parameter for local images
          imageEffectiveSrc = `${imageEffectiveSrc}?v=${Date.now()}`;
        }

        const imgWidthForNextImage = parseInt(commonStyles.width?.toString() || "300", 10) || 300;
        const imgHeightForNextImage = parseInt(commonStyles.height?.toString() || "200", 10) || 200;

        const imageVisualStyles: React.CSSProperties = {
            objectFit: commonStyles.objectFit as CSSProperties['objectFit'] || 'cover',
        ...
        if (isLocalUpload) {
          const plainImgStyles: React.CSSProperties = {
            ...imageVisualStyles,
            width: '100%', 
            height: '100%',
            display: 'block',
          ...
          return (
            <div style={wrapperStyles}>
              <img
                  ...
                  style={plainImgStyles}
                  {...(Object.fromEntries(Object.entries(el.props || {}).filter(([key]) => key.startsWith('data-'))))}
              />
            </div>
          );
        } else {
          return (
            <div style={wrapperStyles}>
                <Image
                    src={imageEffectiveSrc}
                    alt={el.alt || "Изображение"}
                    width={imgWidthForNextImage}
                    height={imgHeightForNextImage}
                    style={imageVisualStyles}
                    className="max-w-full h-auto" // Ensures responsiveness within the wrapper
                    {...el.props}
                    // Add key here if next/image caching becomes an issue for remote URLs too
                    // key={imageEffectiveSrc} 
                />
            </div>
          );
        }
        ...
        >
          {currentElement.children.map(childElement => (
            <RenderedCanvasElement
              ...
              elements={currentElement.children!} 
